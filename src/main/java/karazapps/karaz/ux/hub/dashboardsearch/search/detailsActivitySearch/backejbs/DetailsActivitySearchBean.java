package karazapps.karaz.ux.hub.dashboardsearch.search.detailsActivitySearch.backejbs; 
...
@Stateless(name="karaz/ux/hub/dashboardsearch/search/DetailsActivitySearch", mappedName="karaz/ux/hub/dashboardsearch/search/DetailsActivitySearch") 
@TransactionManagement(TransactionManagementType.BEAN)
@Remote
public class DetailsActivitySearchBean implements KarazQuery {

	private static final int DefaultMaxResults = 10;
	private static final int DefaultFirstResult = 0;
	private static Logger logger = Logger.getLogger(DetailsActivitySearchBean.class); 
	private SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
	XPathExpression<Element> xpath_data_entity = XPathFactory.instance().compile("entity", Filters.element());

	private String searchPath = "karaz/ux/hub/dashboardsearch/search/DetailsActivitySearch";
	...
	public DetailsActivitySearchBean () {
	...
	public TreeMap<String, Object> runQuery(String xml, Integer firstResult, Integer maxResult, AbstractKarazUser user, TreeMap<String, String> context) throws Exception {
		...
		DynamicQueryBuilder dqb = new DynamicQueryBuilder(queryClause, queryData, "karazapps.karaz.ux.hub.dashboardsearch.model.DashboardSearch", ordering, indexationMap);
		dqb.buildQuery();
