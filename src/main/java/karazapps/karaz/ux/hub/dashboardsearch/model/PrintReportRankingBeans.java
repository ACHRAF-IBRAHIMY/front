...
@Stateless(name = "karaz/ux/hub/dashboardsearch/PrintReportRankingBeans", mappedName = "karaz/ux/hub/dashboardsearch/PrintReportRankingBeans")
@Remote
public class PrintReportRankingBeans implements KarazRemoteAction {

	Logger logger = Logger.getLogger(PrintReportRankingBeans.class);

	@Override
	public String execute(String xml, Long doid, TreeMap<String, String> ctx) {
		try {
		String attName = "RAPPORT RANKING";
	     ctx.put("attachmentName", attName);
		 Long ret = BackEndEJBContext.getPrintAction( "karaz/ux/hub/portailsearch/reporting/reportranking/PersoReportrankingReport").print(doid, xml, ctx);
		    if(doid<=0){
