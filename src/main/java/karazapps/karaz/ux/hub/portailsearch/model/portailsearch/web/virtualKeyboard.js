/* ********************************************************************
 **********************************************************************
 * HTML Virtual Keyboard Interface Script 
 
 */



function VKI_buildKeyboardInputs(inp,clc,root,context,typeNom) {
console.log("typeNomINI",typeNom);
var self = this;
this.VKI_version = "";
this.VKI_type = typeNom;
      ...
      if ((ex.nodeName == "TEXTAREA" || ex.type == "text" || ex.type == "password")) {
        if (!ex.id) {
          ...
          ex.id = keyid;
            ...
            clc.onclick = (function(a) { return function() { 
                self.VKI_show(a); 
                for (var i = 0; i < VKI_buildKeyboardInputs.all.length; i++) {
                    if(VKI_buildKeyboardInputs.all[i]!=self){
                        VKI_buildKeyboardInputs.all[i].VKI_close();
       ...
       // ex.parentNode.insertBefore(clc, ex.nextSibling);
...
var logo = document.createElement('span');
      ...
      var clearer = document.createElement('span');
          ...
          clearer.onclick = function() {
            ...
            if(this.VKI_type=="nom"){
                root.donnees.compte.nomar = "";
                context.formRender.notifyObservers("donnees.compte.nomar");
            }else if(this.VKI_type=="prenom"){
                root.donnees.compte.prenomar = "";
                 context.formRender.notifyObservers("donnees.compte.prenomar");
            }
          ...
          var td = document.createElement('td');
            ...
            switch (lkey[1]) {
              ...
              case "AltGr":
                ...
                break;
              case "Tab":
                td.onclick = function() { self.VKI_insert("\t",typeNom); return false; };
                break;
              case "Bksp":
                td.onclick = function() { 
                  self.VKI_target.focus();
                  if (self.VKI_target.setSelectionRange) {
                    ...
                    console.log("self.VKI_target.value",self.VKI_target.value);
                  } 
                  if (self.VKI_shift) self.VKI_modify("Shift");
                  if (self.VKI_alternate) self.VKI_modify("AltGr");
                  
                    console.log("self.VKI_target.value",self.VKI_target.value);
                    console.log("typeNom",typeNom);

                    if(this.VKI_type=="nom"){
                        console.log("typeNom","1"); 
                        root.donnees.compte.nomar = self.VKI_target.value;
                        context.formRender.notifyObservers("donnees.compte.nomar");
                    }else if(this.VKI_type=="prenom"){
                        console.log("typeNom","2");
                        root.donnees.compte.prenomar = self.VKI_target.value;
                         context.formRender.notifyObservers("donnees.compte.prenomar");
                    }
                  return true;
                };
                ...
                td.onclick = function() {
                          ...
                          if (this.firstChild.nodeValue != " ") {
                            for (var z = 0, rezzed = false, dk; dk = self.VKI_deadkey[key][z++];) {
                              if (dk[0] == this.firstChild.nodeValue) {
                                self.VKI_insert(dk[1],typeNom);
                                rezzed = true;
                                break;
                              }
                            }
                          } else {
                            self.VKI_insert(self.VKI_dead,typeNom);
                            rezzed = true;
                          }
                          break;
                        }
                  ...
                  if (!rezzed && this.firstChild.nodeValue != "\xa0") {
                    if (self.VKI_deadkeysOn) {
                      ...
                      if (!self.VKI_dead) self.VKI_insert(this.firstChild.nodeValue,typeNom);
                    } else self.VKI_insert(this.firstChild.nodeValue,typeNom);
                  }
...
this.VKI_insert = function(text,typeNom) {
  ...
  //this.VKI_target.focus();
  console.log("self.VKI_target.value",typeNom);
  console.log("self.VKI_target.donnes",root.donnees); 

 
  if(typeNom=="nom"){
    root.donnees.compte.nomar = this.VKI_target.value;
    context.formRender.notifyObservers("donnees.compte.nomar");
}else if(typeNom=="prenom"){
    root.donnees.compte.prenomar = this.VKI_target.value;
     context.formRender.notifyObservers("donnees.compte.prenomar");
}
...
this.VKI_show = function(id) {
    $(inp).closest(".ow-field-text-container").addClass("filledInput");
  if (this.VKI_target = document.getElementById(id)) {
    if (this.VKI_visible != id) {
      ...
      //var divBf = this.VKI_INP[0].parentNode.parentNode.parentNode.getElementsByClassName("ow-field-input-line")[0];

        this.VKI_INP[0].parentNode.parentNode.parentNode.appendChild(this.VKI_keyboard);
      //this.VKI_INP[0].parentNode.parentNode.parentNode.insertBefore(divBf, this.VKI_INP[0].parentNode.parentNode.nextSibling);
...
} else if (self.VKI_INP[0].parentNode.parentNode.parentNode)
  return [self.VKI_INP[0].parentNode.parentNode.parentNode.clientWidth, self.VKI_INP[0].parentNode.parentNode.parentNode.clientHeight];
return [0, 0];
}
...
}/* from file karazapps/rokhasactivite/activite/model/activite/web/Activite.js  */
function isValueNotBiggerThan(value,length){
try{
    ...
    console.log("ERROR in Javascript function isValueNotSmallerThan(value,length) .......");
}		
}

function isEmptyy(value1,value2,value3,value4){
try{
    ...
    console.log("ERROR in Javascript function isEmpty) .......");
}		
}

function isValueAllArabicLetters(value){
try{
    var reg = /^[\u0621-\u064A ]+$/;
    if( reg.test(value)){
        return true;
    ...
    console.log("ERROR in Javascript function isValueAllArabicLetters(value) .......");
}
}
